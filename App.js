...
import { MensIcon, WomensIcon, ShoesIcon, BagsIcon } from './CategoryIcons';
...
// صريح بدل الاعتماد على تفعيل RTL التلقائي (شوف ملاحظة bottomNav تحت).
// الأيقونات custom SVG (line-art) مطابقة لتصميم البانر، مش من مكتبة جاهزة.
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* الصورة مصمّمة كاملة (نص + عناصر) خارج الكود، فمفيش تراكب
            ...
            المطبوع في الصورة عشان يفضل قابل للضغط. */}
              ...
              <React.Fragment key={cat.id}>
                <TouchableOpacity style={styles.quickCatItem} activeOpacity={0.7}>
                  <View style={styles.quickCatIconWrap}>
                    <cat.Icon size={26} color={COLORS.burgundy} />
                  </View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <View>
              <Text style={styles.sectionLabel}>الأكثر تميزاً</Text>
              ...
              <View key={p.id} style={styles.featuredCard}>
                <View style={styles.featuredImageWrap}>
                  ...
                  <TouchableOpacity style={styles.featuredHeartBtn}>
                    <Text style={styles.featuredHeartIcon}>♡</Text>
                  </TouchableOpacity>
                ...
                <TouchableOpacity style={styles.featuredAddBtn}>
                  <Text style={styles.featuredAddBtnText}>أضف للسلة 🛍️</Text>
                </TouchableOpacity>
              </View>
            ))}
          </View>
...
const styles = StyleSheet.create({
  ...
  // Quick categories (icon row) — كارت أبيض عائم بحواف مدورة، دواير
  // بيج فاتحة للأيقونات، فواصل رأسية رفيعة بين العناصر. row-reverse
  // صريح (بدل الاعتماد على RTL التلقائي) عشان الترتيب البصري يفضل
  // ثابت بغض النظر عن حالة I18nManager وقت الفتح.
  quickCatWrap: { paddingHorizontal: 20, marginTop: -18, marginBottom: 4 },
  quickCatCard: {
  ...
  quickCatLabel: { fontFamily: THEME.fontBody, fontSize: 12, color: THEME.textPrimary },
  quickCatDivider: { width: 1, height: 36, backgroundColor: THEME.border, alignSelf: 'center' },
  ...
  // البانر الجديد: صورة واحدة كاملة (النص والعناصر مطبوعين فيها أصلاً)
  // بنسبة أبعاد ثابتة 989:441 عشان تتمدد صح على أي عرض شاشة من غير
  // ما تتقص أو تتمطط.
  heroBannerImage: { width: '100%', aspectRatio: 989 / 441 },
  // منطقة ضغط شفافة فوق مكان زرار "اكتشف المجموعة" المطبوع في الصورة
  // (نسب مئوية من أبعاد البانر عشان تفضل متمركزة صح مهما كان عرض الشاشة)
  heroBannerCtaHit: {
    position: 'absolute', left: '9%', top: '62%', width: '24%', height: '14%',
  },
  ...
  // Categories
  // Featured Products (منتجات مميزة) — بطاقة بيضاء مرفوعة الحواف،
  // قلب مفضّلة دائري أعلى اليسار، بادج فئة (نسائية/رجالية) أعلى
  // اليمين، وصف قصير تحت العنوان، وزرار ذهبي كامل العرض بدل زرار
  // الـ "+" الصغير المستخدم في سكشن "القطع الجديدة" الديناميكي.
  featuredGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: CARD_GAP, justifyContent: 'space-between' },
  featuredCard: { width: CARD_W, marginBottom: 20, backgroundColor: THEME.card, borderRadius: 10, padding: 10 },
  featuredImageWrap: {
    width: '100%', height: CARD_W * 1.15, backgroundColor: THEME.background,
    marginBottom: 12, position: 'relative', borderRadius: 8, overflow: 'hidden',
  },
  ...
  featuredHeartIcon: { fontSize: 15, color: COLORS.burgundy },
  featuredBadge: {
  ...
  featuredSubtitle: { fontFamily: THEME.fontBody, fontSize: 11.5, color: THEME.textSecondary, marginBottom: 6 },
  featuredPrice: { fontFamily: THEME.fontBodySemiBold, fontSize: 15, color: THEME.textPrice, marginBottom: 10 },
  featuredAddBtn: {
    backgroundColor: COLORS.gold, borderRadius: 8, paddingVertical: 11,
    alignItems: 'center', justifyContent: 'center',
  },
  featuredAddBtnText: { fontFamily: THEME.fontBodySemiBold, color: COLORS.burgundy, fontSize: 13 },
  ...
  bottomNavActive: { color: COLORS.gold, fontFamily: THEME.fontBodySemiBold },
  // شعار الروبوت في البار السفلي: صورة 3D عالية الجودة الآن، فكبّرناه
  // شوية كمان (36 → 46) عشان التفاصيل (السماعة، الفيونكة) تبان واضحة
  // ومتناسقة بدل ما تتقزّم في مساحة صغيرة.
  ...
  bottomNavBotIcon: { width: 46, height: 46 },
});
