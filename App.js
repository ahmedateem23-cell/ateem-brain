...
// خلّي المتصفح المؤقت بتاع تسجيل الدخول يقفل نفسه لوحده بعد الموافقة
// (خطوة لازمة مع expo-auth-session).
WebBrowser.maybeCompleteAuthSession();

// ⚠️ لازم تستخرج الـ Client IDs دي من Google Cloud Console
// (APIs & Services → Credentials → OAuth 2.0 Client IDs) وتحطها هنا —
// بدونها زرار "الاشتراك عبر Google" مش هيشتغل فعليًا. محتاج كمان
// تتأكد إنك مثبّت الباكدجين دول: expo-auth-session و expo-web-browser
// (npx expo install expo-auth-session expo-web-browser).
const GOOGLE_WEB_CLIENT_ID = '477169698019-krvglqdb1uhaiptma9atd4a6i8s5fvn7.apps.googleusercontent.com';
const GOOGLE_IOS_CLIENT_ID = '477169698019-inlp0n0iv0h1gq75ajb9haqb2n4fq2cb.apps.googleusercontent.com';
const GOOGLE_ANDROID_CLIENT_ID = '477169698019-p55usho037t847sfnantnhk2vg6ohk7t.apps.googleusercontent.com';
...
// عرض القائمة الجانبية الجديدة — 78% من عرض الشاشة، بيسيب مساحة
// كافية تبان فيها خلفية المتجر معتّمة خلفها.
...
// أيقونة Google الرسمية (G متعددة الألوان) — نفس المسارات المعتمدة في
// أزرار "تسجيل الدخول عبر Google" القياسية، عشان الزرار يبان موثوق
// ومطابق لهوية Google البصرية زي ما طلبت المواصفات ("Google icon
// رسمي وبسيط").
...
export default function App() {
  ...
  const menuTranslateX = useRef(new Animated.Value(-DRAWER_WIDTH)).current;
  const menuBackdropOpacity = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    if (menuOpen) {
      setMenuModalVisible(true);
      menuTranslateX.setValue(-DRAWER_WIDTH);
      menuBackdropOpacity.setValue(0);
      Animated.parallel([
        Animated.timing(menuBackdropOpacity, { toValue: 1, duration: 240, useNativeDriver: true }),
        Animated.spring(menuTranslateX, { toValue: 0, useNativeDriver: true, damping: 20, stiffness: 180, mass: 0.9 }),
      ]).start();
    }
  }, [menuOpen]);
  ...
  function closeMenu() {
    Animated.parallel([
      Animated.timing(menuBackdropOpacity, { toValue: 0, duration: 200, useNativeDriver: true }),
      Animated.timing(menuTranslateX, { toValue: -DRAWER_WIDTH, duration: 220, useNativeDriver: true }),
    ]).start(() => {
  ...
  // مفيش حركة نبض مستمرة على الزرار العائم دلوقتي (كانت هنا قبل كده)
  // — المواصفات صريحة: "لا يجعل البوت يتحرك أو يهتز بشكل ملفت" و
  // "ممنوع: Pulsing قوي / عناصر تتحرك باستمرار". الحركة الوحيدة
  // المتبقية هي "soft scale" خفيفة جدًا عند الضغط بس (botBounce تحت).
  const botBounce = useRef(new Animated.Value(1)).current;
  function handleBotPress() {
    Animated.sequence([
      Animated.timing(botBounce, { toValue: 0.9, duration: 90, useNativeDriver: true }),
      Animated.spring(botBounce, { toValue: 1, friction: 5, tension: 140, useNativeDriver: true }),
    ]).start();
    setChatOpen(true);
  }
  ...
  // ===== اشتراك النشرة البريدية عبر Google =====
  // بدل ما العميل يكتب بريده يدويًا، بنستخدم Google Sign-In للحصول
  // على البريد تلقائيًا بعد موافقته، ونبعته لنفس الـ endpoint القديم
  // (newsletter-subscribe) في الـ proxy.
  ...
  useEffect(() => {
    if (googleResponse?.type === 'success') {
      ...
      subscribeWithGoogleAccessToken(accessToken);
    } else if (googleResponse?.type === 'error') {
      setNewsletterStatus('error');
    }
    // لو المستخدم لغى نافذة Google (type === 'cancel') منسيبش رسالة
    // خطأ — ده مش فشل فعلي، هو بس رجع من غير ما يكمل.
  }, [googleResponse]);

  async function subscribeWithGoogleAccessToken(accessToken) {
    if (!accessToken) {
      setNewsletterStatus('error');
      return;
    }
    ...
    try {
      ...
      const res = await fetch(ATEEM_PROXY_URL + 'newsletter-subscribe', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: profile.email, source: 'google' }),
      });
  ...
  function handleGoogleNewsletterPress() {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <Modal visible={menuModalVisible} transparent animationType="none" onRequestClose={closeMenu}>
        <View style={{ flex: 1 }}>
          ...
          <Animated.View style={[styles.menuPanel, { transform: [{ translateX: menuTranslateX }] }]}>
            <View style={styles.menuPanelHeader}>
              <View>
                <Text style={styles.menuPanelBrand}>ATEEM</Text>
                <Text style={styles.menuPanelTagline}>SUDAN · MODERN LUXURY</Text>
              </View>
              <TouchableOpacity style={styles.menuCloseBtn} onPress={closeMenu} activeOpacity={0.75}>
                <Text style={styles.menuCloseBtnText}>×</Text>
              </TouchableOpacity>
            </View>
            <View style={styles.menuPanelDivider} />

            <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.menuLinksWrap} showsVerticalScrollIndicator={false}>
              {[...NAV_LINKS, 'تواصل معنا'].map((link) => (
                <TouchableOpacity key={link} style={styles.menuLinkRow} onPress={closeMenu} activeOpacity={0.7}>
            ...
            <View style={styles.menuPanelFooter}>
              <Text style={styles.menuFooterText}>ATEEM © 2026</Text>
            </View>
          </Animated.View>
        </View>
      </Modal>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
              ...
              <View key={p.id} style={[styles.featuredCard, { width: CARD_W }]}>
                <View style={styles.featuredImageWrap}>
                  ...
                  <TouchableOpacity style={styles.featuredHeartBtn}>
                    <HeartIcon size={16} color={COLORS.burgundy} />
                  </TouchableOpacity>
                ...
                <Text style={[styles.featuredPrice, styles.featuredPriceSmall]}>{p.price}</Text>
            ...
            <View style={styles.featuredGrid}>
              {newArrivals.map((p) => (
                <View key={p.id} style={[styles.featuredCard, { width: CARD_W }]}>
                  ...
                  <Text style={[styles.featuredPrice, styles.featuredPriceSmall]}>{fmtOdooPrice(p)}</Text>
                  <TouchableOpacity style={styles.featuredAddBtn}>
                    <Text style={styles.featuredAddBtnText}>أضف للسلة</Text>
                    <BagsIcon size={12} color={COLORS.burgundy} />
                  </TouchableOpacity>
          ...
          {LOOKS.map((look) => (
            <TouchableOpacity key={look.id} style={styles.lookItem}>
              ...
              <View style={styles.lookOverlay}>
                ...
                <View style={styles.lookUnderline} />
        ...
        <View style={styles.notifyCard}>
          ...
          <Text style={styles.notifyDesc}>كن جزءًا من عالم ATEEM وتعرّف على المجموعات الجديدة والعروض المختارة بعناية.</Text>

          <TouchableOpacity
            style={[styles.notifyGoogleBtn, newsletterStatus === 'sending' && { opacity: 0.7 }]}
            onPress={handleGoogleNewsletterPress}
            disabled={newsletterStatus === 'sending' || !googleRequest}
            activeOpacity={0.85}
          >
            {newsletterStatus === 'sending' ? (
              ...
              <>
                <GoogleGIcon size={17} />
                <Text style={styles.notifyGoogleBtnText}>الاشتراك عبر Google</Text>
              </>
            )}
          </TouchableOpacity>

          {newsletterStatus === 'success' && (
            <Text style={styles.notifySuccessText}>تم الاشتراك بنجاح ✦</Text>
          )}
          {newsletterStatus === 'error' && (
            <Text style={styles.notifyErrorText}>تعذّر إتمام الاشتراك عبر Google، حاول مرة أخرى.</Text>
          )}
        </View>
      </ScrollView>
      ...
      {!chatOpen && (
        <Animated.View
          style={[styles.floatingBotAnchor, { transform: [{ scale: botBounce }] }]}
        >
          <TouchableOpacity
            ...
            activeOpacity={0.85}
          >
            <Text style={styles.floatingBotMark}>A</Text>
          </TouchableOpacity>
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
  ...
  // القائمة الجانبية الجديدة — درج (drawer) كامل الارتفاع بيدخل من
  // الشمال (نفس جهة أيقونة المنيو في الهيدر)، بهوية ألوان البراند
  // كاملة بدل الكارت الأبيض البسيط القديم.
  menuBackdrop: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(20,6,10,0.55)' },
  menuPanel: {
    position: 'absolute', left: 0, top: 0, bottom: 0, width: DRAWER_WIDTH,
    backgroundColor: THEME.card,
    borderTopRightRadius: 24, borderBottomRightRadius: 24,
    overflow: 'hidden',
    shadowColor: '#000', shadowOffset: { width: 4, height: 0 }, shadowOpacity: 0.25, shadowRadius: 16, elevation: 20,
  },
  menuPanelHeader: {
    backgroundColor: COLORS.burgundy,
  ...
  menuPanelBrand: { fontFamily: THEME.fontHeading, color: COLORS.gold, fontSize: 22, letterSpacing: 2 },
  menuPanelTagline: { fontFamily: THEME.fontBody, color: COLORS.gold, fontSize: 8.5, letterSpacing: 1.3, marginTop: 4 },
  menuCloseBtn: {
  ...
  menuCloseBtnText: { color: COLORS.gold, fontSize: 18, fontFamily: THEME.fontBodySemiBold, marginTop: -1 },
  menuPanelDivider: { height: StyleSheet.hairlineWidth, backgroundColor: 'rgba(201,169,97,0.5)' },
  menuLinksWrap: { paddingHorizontal: 20, paddingTop: 16, paddingBottom: 12 },
  menuLinkRow: {
    flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'flex-end', gap: 8,
    paddingVertical: 15, borderBottomWidth: 1, borderBottomColor: THEME.border,
  },
  menuLinkText: { fontFamily: THEME.fontHeading, fontSize: 16, color: THEME.textPrimary, textAlign: 'right' },
  menuLinkArrow: { fontFamily: THEME.fontBody, fontSize: 15, color: COLORS.gold },
  menuPanelFooter: {
    paddingHorizontal: 20, paddingVertical: 16,
    borderTopWidth: 1, borderTopColor: THEME.border,
  },
  menuFooterText: { fontFamily: THEME.fontBody, fontSize: 9.5, color: THEME.textSecondary, textAlign: 'center', letterSpacing: 0.5 },
  ...
  heroCtaBtnText: { fontFamily: THEME.fontBodySemiBold, color: COLORS.gold, fontSize: 11.5, letterSpacing: 0.4 },
  heroCtaBtnArrow: { fontFamily: THEME.fontBodySemiBold, color: COLORS.gold, fontSize: 13 },
  heroTitleItalic: { fontStyle: 'italic', color: COLORS.burgundy },

  // paddingVertical اتزوّد شوية (20→26) وmarginBottom بتاعة عنوان
  // القسم كمان (16→20) — مسافات تنفّس أكبر بين الأقسام بالظبط زي
  // "Less is More" في البريف، من غير ما نلمس paddingHorizontal (ده
  // مرتبط بحساب عرض البطاقات CARD_W وميتغيرش).
  section: { paddingHorizontal: 20, paddingVertical: 26 },
  sectionHeader: { flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 20 },
  ...
  // نسخة أصغر من السعر — بقت مستخدمة في القسمين (منتجات مميزة
  // والقطع الجديدة) بعد ما شلنا سطر الوصف من الاتنين.
  featuredPriceSmall: { fontSize: 12.5, marginTop: 2 },
  featuredAddBtn: {
  ...
  lookSeason: {
    fontFamily: THEME.fontBody, color: 'rgba(201,169,97,0.9)', fontSize: 9.5, letterSpacing: 1.2,
    textAlign: 'right', textTransform: 'uppercase',
  },
  lookUnderline: { width: 30, height: 1.5, borderRadius: 1, backgroundColor: COLORS.gold, marginVertical: 7 },
  lookFooterBtn: {
  ...
  // زرار "الاشتراك عبر Google" الوحيد — خلفية بيضاء نظيفة (زي أزرار
  // Google الرسمية القياسية) بدل صندوق إدخال بريد يدوي + زرار منفصل.
  notifyGoogleBtn: {
  ...
  notifyGoogleBtnText: { fontFamily: THEME.fontBodySemiBold, color: COLORS.burgundy, fontSize: 13 },
  notifySuccessText: { fontFamily: THEME.fontBody, color: COLORS.gold, fontSize: 11.5, textAlign: 'center', marginTop: 10 },
  notifyErrorText: { fontFamily: THEME.fontBody, color: '#F0AFAF', fontSize: 11.5, textAlign: 'center', marginTop: 10 },

  bottomNav: {
    flexDirection: 'row-reverse', height: 56, backgroundColor: '#FFFDF9',
    borderTopWidth: 1, borderTopColor: THEME.border,
  ...
  // الزرار العائم بقى "ATEEM Concierge" راقٍ: دايرة زجاجية عاجية صغيرة
  // (46px) بحد ذهبي رفيع جدًا، من غير أي شادو خالص (زي المطلوب) —
  // بدون أي شكل روبوت/Mascot، وبدون Glow قوي. جوّاها شعار المتجر
  // (حرف "A" بخط Playfair Display) بحجم أصغر من قبل، بلون عنابي.
  floatingBotAnchor: {
    position: 'absolute',
    bottom: 70,
    left: 14,
  ...
  floatingBot: {
    ...
    borderWidth: 1, borderColor: 'rgba(201,169,97,0.55)',
    alignItems: 'center', justifyContent: 'center',
  },
  floatingBotMark: {
    fontFamily: THEME.fontHeading, fontSize: 14, color: COLORS.burgundy, letterSpacing: 0.5,
  },
});
