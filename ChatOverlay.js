...
const [WELCOME_TITLE, WELCOME_SUBTITLE] = WELCOME_TEXT.split('\n');
                ...
                <View key={m.id} style={{ marginBottom: 10 }}>
                  ...
                  {m.role === 'bot' && m.isWelcome && (
                    <View style={cs.welcomeWrap}>
                      <View style={cs.welcomeCard}>
                        <View style={cs.welcomeDivider} />
                        <Text style={cs.welcomeTitle}>{WELCOME_TITLE}</Text>
                        <Text style={cs.welcomeSubtitle}>{WELCOME_SUBTITLE}</Text>
                      </View>
                    </View>
                  )}
                  {m.role === 'bot' && !m.isWelcome && (
...
const cs = StyleSheet.create({
  ...
  // صورة البوت في الهيدر: كبّرناها ومن غير إطار ذهبي (بناءً على الطلب) —
  // سبنا بس الخلفية الفاتحة + الظل الخفيف عشان تفضل بارزة وواضحة فوق
  // العنابي الغامق حتى من غير برواز.
  logoImgWrap: {
  ...
  // I18nManager وقت الفتح.
  subRow: { flexDirection: 'row-reverse', alignItems: 'center', marginTop: 3, gap: 5 },
  ...
  // رسالة الترحيب: كارت مميز في النص، بلا ذيل بابل زي باقي الرسايل،
  // بخط فاصل ذهبي وعنوان أكبر — عشان تبان كـ "بداية تجربة" مش رسالة
  // شات عادية.
  welcomeWrap: { alignItems: 'center', width: '100%', marginBottom: 4 },
  welcomeCard: {
  ...
  welcomeDivider: { width: 36, height: 2, backgroundColor: COLORS.gold, borderRadius: 1, marginBottom: 12 },
  welcomeTitle: { fontFamily: THEME.fontHeading, fontSize: 18, color: COLORS.burgundy, textAlign: 'center', marginBottom: 8 },
  welcomeSubtitle: { fontFamily: THEME.fontBody, fontSize: 13.5, color: THEME.textPrimary, textAlign: 'center', lineHeight: 22 },
