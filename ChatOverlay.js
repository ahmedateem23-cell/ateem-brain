...
const WELCOME_TITLE = 'أهلاً بك في ATEEM.';
const WELCOME_SUB = 'أنا هنا لأساعدك في اختيار ما يناسب ذوقك.';
const WELCOME_PROMPT = 'ماذا تبحث اليوم؟';
// أربعة خيارات بس (بدل سبعة) — شيلنا "للرجال/للنساء" لأنها موجودة في
// تنقل المتجر أصلاً، والـConcierge لازم يقدّم قيمة إضافية مش يكررها.
// "primary: true" هو الخيار الرئيسي (ساعدني في اختيار إطلالتي) —
// بروزه شوية أكتر (خلفية/حد Burgundy) بينما الباقي Ivory بسيط.
const UTILITY_ACTIONS = [
  { label: 'اكتشف المجموعة', text: 'أرني أبرز المنتجات المتوفرة عندكم' },
  { label: 'ساعدني في اختيار إطلالتي', text: 'ساعدني في اختيار إطلالة تناسبني', primary: true },
  { label: 'المقاسات والتفاصيل', text: 'أحتاج مساعدة في المقاسات' },
...
export default function ChatOverlay({ visible, onClose, products }) {
  ...
  const [messages, setMessages] = useState([
    { id: nextMsgId(), role: 'bot', text: WELCOME_TITLE + ' ' + WELCOME_SUB, time: nowTime(), productIds: [], isWelcome: true },
  ]);
          ...
          <View style={cs.panel}>
            ...
            <View style={cs.header}>
              <View style={cs.headerContent}>
                <View style={cs.logoImgWrap}>
                  {/* شعار ATEEM الأصلي بدل حرف A النصي — لازم تحط ملف
                      الصورة في مجلد ./assets باسم ateem-logo.png (مرفق
                      لك في التحميلات) عشان الـ require يلاقيه. */}
                  <Image source={require('./assets/ateem-logo.png')} style={cs.logoImg} />
                </View>
                <View>
                  <Text style={cs.title}>ATEEM Concierge</Text>
                  <View style={cs.subRow}>
                    <View style={cs.statusDot} />
                    <Text style={cs.subText}>متاح الآن</Text>
                  </View>
                </View>
              </View>
                ...
                <View key={m.id} style={{ marginBottom: 10 }}>
                  ...
                  {m.role === 'bot' && m.isWelcome && (
                    <View style={cs.welcomeWrap}>
                      <Text style={cs.welcomeTitle}>{WELCOME_TITLE}</Text>
                      <Text style={cs.welcomeText}>{WELCOME_SUB}</Text>
                      <Text style={cs.welcomePrompt}>{WELCOME_PROMPT}</Text>
                    </View>
                  )}
                  {m.role === 'bot' && !m.isWelcome && (
              ...
              {messages.length <= 1 && (
                <View style={cs.quickWrap}>
                  {UTILITY_ACTIONS.map((a) => (
                    <TouchableOpacity
                      key={a.label}
                      style={[cs.quickChip, a.primary && cs.quickChipPrimary]}
                      onPress={() => handleSend(a.text)}
                      activeOpacity={0.7}
                    >
                      <Text style={[cs.quickChipText, a.primary && cs.quickChipTextPrimary]}>{a.label}</Text>
                    </TouchableOpacity>
                  ))}
                  {/* دعوة صغيرة وهادئة بس، مش زرار — بتفتح لوحة المفاتيح
                      مباشرة للي عايز يكتب طلبه بنفسه بدل ما يختار. */}
                  <TouchableOpacity onPress={() => inputRef.current?.focus()} activeOpacity={0.6}>
                    <Text style={cs.freeTextHint}>أو أخبرني بما تبحث عنه...</Text>
                  </TouchableOpacity>
                </View>
              )}
            </ScrollView>
            ...
            <View style={cs.inputRow}>
              ...
              <TextInput
                ...
                placeholder={isRecording ? 'بنسمعك...' : 'أخبرني بما تبحث عنه...'}
...
const cs = StyleSheet.create({
  ...
  // بدّلنا الظل القوي بحد ذهبي رفيع بس — أقرب لروح "Minimal" المطلوبة
  // بدل أي glow أو ظل ثقيل حوالين الشعار.
  logoImgWrap: {
  ...
  logoGlyph: { fontFamily: THEME.fontHeading, color: COLORS.burgundy, fontSize: 17 },
  title: { fontFamily: THEME.fontHeading, color: COLORS.white, fontSize: 14 },
  ...
  welcomeWrap: { alignItems: 'center', width: '100%', marginBottom: 4, paddingHorizontal: 12, paddingTop: 16 },
  welcomeTitle: { fontFamily: THEME.fontHeading, fontSize: 17, color: COLORS.burgundy, textAlign: 'center', lineHeight: 25 },
  welcomeText: { fontFamily: THEME.fontBody, fontSize: 13, color: THEME.textSecondary, textAlign: 'center', lineHeight: 20, marginTop: 4 },
  welcomePrompt: { fontFamily: THEME.fontBodySemiBold, fontSize: 13, color: THEME.textPrimary, textAlign: 'center', lineHeight: 20, marginTop: 10 },
  ...
  // بديل النجمتين ** ** — نفس حجم الخط بس بلون ذهبي ووزن أثقل، يبرز
  // اسم منتج أو تفصيلة مهمة جوه رسالة البوت من غير رموز ماركداون ظاهرة.
  botTextEmphasis: { fontFamily: THEME.fontBodySemiBold, color: COLORS.gold, fontSize: 13.5, lineHeight: 21 },
  ...
  // قائمة عمودية بسيطة بدل شبكة/شرائط أفقية — أقرب لإحساس Concierge
  // راقٍ من "أزرار شات" عادية. كل خيار سطر كامل العرض بحد رفيع فاتح.
  quickWrap: { width: '100%', gap: 8, marginTop: 14 },
  quickChip: {
    borderWidth: 1, borderColor: THEME.border, backgroundColor: THEME.card,
    paddingVertical: 12, paddingHorizontal: 16, borderRadius: 12,
    alignItems: 'center',
  },
  quickChipText: { fontFamily: THEME.fontBody, fontSize: 13, color: COLORS.burgundy, textAlign: 'center' },
  // الخيار الرئيسي ("ساعدني في اختيار إطلالتي") — خلفية Burgundy بدل
  // الحد الرفيع، عشان يبرز شوية عن باقي الخيارات من غير ما نضيف ألوان
  // جديدة أو Glow.
  quickChipPrimary: { backgroundColor: COLORS.burgundy, borderColor: COLORS.burgundy },
  quickChipTextPrimary: { color: COLORS.white, fontFamily: THEME.fontBodySemiBold },
  // دعوة نصية صغيرة وهادئة تحت الأزرار — مش زرار، بس بتفتح لوحة
  // المفاتيح مباشرة لمن يفضّل يكتب طلبه بنفسه.
  freeTextHint: {
    fontFamily: THEME.fontBody, fontSize: 11.5, color: THEME.textSecondary,
  ...
  input: {
    flex: 1, backgroundColor: THEME.background, borderRadius: 19,
    paddingHorizontal: 14, paddingVertical: 8, fontFamily: THEME.fontBody, fontSize: 13, color: THEME.textPrimary,
  },
  micBtn: {
    width: 36, height: 36, borderRadius: 18,
  ...
  sendBtn: {
    width: 36, height: 36, borderRadius: 18, backgroundColor: COLORS.burgundy,
    alignItems: 'center', justifyContent: 'center',
  },
  sendBtnText: { color: COLORS.white, fontSize: 15, transform: [{ scaleX: -1 }] },
});
