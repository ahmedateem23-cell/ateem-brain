...
const WELCOME_PROMPT = 'أخبرني بما تبحث عنه.';
// خيارات الشاشة الأولى — قائمة راقية عمودية (مش شبكة أيقونات)، بتظهر
// بس قبل ما المحادثة تبدأ. "استكشف كضيف" بيسمح للعميل يتصفح من غير ما
// نطلب تسجيل دخول؛ التسجيل بيتطلب بس وقت إتمام الطلب فعليًا.
const UTILITY_ACTIONS = [
  { label: 'اكتشف المجموعة', text: 'أرني أبرز المنتجات المتوفرة عندكم' },
  { label: 'للرجال', text: 'أرني تشكيلة الرجال' },
  { label: 'للنساء', text: 'أرني تشكيلة النساء' },
  { label: 'الإكسسوارات', text: 'أرني الإكسسوارات المتوفرة' },
  { label: 'المقاسات والتفاصيل', text: 'أحتاج مساعدة في المقاسات' },
  { label: 'تتبع طلبي', text: 'أريد تتبع طلبي' },
  { label: 'استكشف كضيف', text: 'أرغب في التصفح فقط حالياً' },
];
            ...
            <View style={cs.header}>
              <View style={cs.headerContent}>
                <View style={cs.logoImgWrap}>
                  {/* رمز هندسي Minimal مستوحى من حرف A في شعار ATEEM —
                      بدل شخصية الروبوت الكرتونية القديمة. */}
                  <Text style={cs.logoGlyph}>A</Text>
                </View>
                <View>
              ...
              {messages.length <= 1 && (
                <View style={cs.quickWrap}>
                  {UTILITY_ACTIONS.map((a) => (
                    <TouchableOpacity key={a.label} style={cs.quickChip} onPress={() => handleSend(a.text)} activeOpacity={0.7}>
                      <Text style={cs.quickChipText}>{a.label}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              )}
...
const cs = StyleSheet.create({
  ...
  welcomeWrap: { alignItems: 'center', width: '100%', marginBottom: 4, paddingHorizontal: 12, paddingTop: 10 },
