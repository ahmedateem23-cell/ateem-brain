...
// الشات بقى "بطاقة عائمة" بحجم وموضع ثابتين — مطابق لمقاس ويدجت الويب
// المرجعي (#ateem-panel: left/right margin ثابت + top/bottom offset
// ثابت + زوايا مدوّرة بالكامل)، مش بانل "مرسو" (docked) مالي الشاشة
// زي قبل. ده بيحل مشكلة إن البانل كان "بيتمدد"/يتقلّص بشكل غريب وقت
// ظهور الكيبورد: دلوقتي حجم البطاقة (PANEL_HEIGHT) ثابت تمامًا طول
// الوقت، ولما الكيبورد يظهر إحنا بنرفع البطاقة كلها لفوق (translateY)
// بدل ما نقلّص ارتفاعها من تحت.
const STATUS_BAR_HEIGHT = Platform.OS === 'android' ? (RNStatusBar.currentHeight || 24) : 44;
// هامش جانبي ثابت (زي left/right:14px في نسخة الويب).
const PANEL_SIDE_MARGIN = 14;
// أقصى عرض للبطاقة على الشاشات الكبيرة (تابلت) — زي max-width:400px
// تقريبًا في نسخة الويب، عشان الشات ما يبقاش عريض أوي على تابلت.
const PANEL_MAX_WIDTH = 400;
// ارتفاع هيدر المتجر العام (navbar) تقريبًا: paddingVertical:10×2 +
// ارتفاع محتوى (اسم "ATEEM" + الشعار الفرعي) ≈ 63px. الشات دلوقتي
// بيبدأ مباشرة من تحت الهيدر ده بدل مسافة عشوائية من أعلى الشاشة.
const STORE_HEADER_HEIGHT = 63;
// مسافة ثابتة من أعلى الشاشة قبل بداية البطاقة = ارتفاع status bar +
// ارتفاع هيدر المتجر العام بالظبط، عشان الشات يبدأ من أسفل هيدر
// المتجر مباشرة زي المطلوب.
const PANEL_TOP_OFFSET = STATUS_BAR_HEIGHT + STORE_HEADER_HEIGHT;
// مسافة ثابتة من أسفل الشاشة قبل نهاية البطاقة — زي bottom:90px في
// نسخة الويب.
const PANEL_BOTTOM_OFFSET = 90;
// زوايا مدوّرة بالكامل (الأربع زوايا) — زي border-radius:28px في نسخة
// الويب، بدل الزوايا العلوية المربّعة القديمة بتاعة وضع الـ docking.
const PANEL_BORDER_RADIUS = 28;
const SCREEN_HEIGHT = Dimensions.get('window').height;
// ارتفاع البطاقة الفعلي — رقم ثابت واحد يتحسب مرة واحدة، مش flex:1
// بيمتلئ لآخر الشاشة، عشان الحجم يفضل زي ما هو بالظبط زي نسخة الويب.
const PANEL_HEIGHT = SCREEN_HEIGHT - PANEL_TOP_OFFSET - PANEL_BOTTOM_OFFSET;
// أقصى مسافة مسموح نرفع بيها البطاقة كلها فوق وقت ظهور الكيبورد، عشان
// حافتها العلوية ما تقربش أوي من status bar. استخدمنا هنا نفس أسلوب
// "رفع البطاقة كلها بـ translateY" المُثبت استقراره (بدل تحريك height
// مباشرة، اللي سبب باگ اختفاء/تلاشي الرسايل وقت الإرسال والاستقبال).
const MAX_KEYBOARD_SHIFT = Math.max(0, PANEL_TOP_OFFSET - 8);
...
const WELCOME_TEXT = 'مرحبًا بك في ATEEM\nكيف يمكنني مساعدتك؟';

// شريط الإجراءات السريعة الملحق برسالة الترحيب — أزرار pill نصية بس
// (بلا أيقونات). تلاتة أزرار بس في صف عرضي واحد بعرض متساوٍ (اتشالت
// "مساعدة في اختيار المقاس" بناءً على الطلب).
const UTILITY_ACTIONS = [
  { label: 'اكتشف المنتجات', text: 'أرني أبرز المنتجات المتوفرة عندكم' },
  { label: 'تتبع طلبي', text: 'أريد تتبع طلبي' },
  { label: 'العروض الحالية', text: 'ما هي العروض الحالية؟' },
];
...
export default function ChatOverlay({ visible, onClose, products }) {
  ...
  // ===== رفع البطاقة كاملة فوق الكيبورد، بدل ما نقلّص ارتفاعها =====
  // (رجّعنا الأسلوب المُثبت ده تحديدًا لأن تحريك height/padding مباشرة
  // سبب باگ اختفاء/"تلاشي" الرسايل وقت الإرسال والاستقبال — أي تحريك
  // JS-driven لخاصية layout زي height أو paddingBottom على حاوية فيها
  // ScrollView بيتصادم مع تحديث محتوى الشات نفسه في نفس اللحظة على
  // أندرويد). حجم البطاقة (PANEL_HEIGHT) ثابت دايمًا. لما الكيبورد
  // يظهر، بنحسب بس قد ايه هو مغطّي من المساحة السفلية المحجوزة أصلاً
  // (PANEL_BOTTOM_OFFSET) ولو غطّاها بنرفع البطاقة كلها لفوق بنفس
  // الفرق — وبقى القيمة سالبة مباشرة (بدل ما نطرحها لاحقًا بـ
  // Animated.subtract) عشان نضيفها كـ transform مستقل تاني، مش عقدة
  // مشتقة (derived node) بتجمع قيمتين native-driven مع بعض — ده أأمن
  // على أندرويد ومتوافق مع نفس مبدأ تجنب الباگ فوق.
  ...
  useEffect(() => {
    ...
    const onShow = (e) => {
      ...
      const neededShift = Math.max(0, keyboardHeight - PANEL_BOTTOM_OFFSET);
      const clamped = Math.min(neededShift, MAX_KEYBOARD_SHIFT);
      // مباشر وسريع (مش بطيء) — "يعلو الكيبورد فقط مباشرة" زي المطلوب.
      Animated.timing(keyboardShift, {
        toValue: -clamped,
        duration: 120,
        useNativeDriver: true,
      }).start();
      // إخفاء أزرار النظام الثلاثة (رجوع/هوم/الأخيرة) طول ما الكيبورد
      // ظاهر — بترجع تلقائيًا لما الكيبورد يقفل (onHide تحت) أو الشات
      // يتقفل (requestClose تحت). المكتبة بقت مضافة في package.json.
      NavigationBar.setVisibilityAsync('hidden').catch(() => {});
    };
    const onHide = () => {
      Animated.timing(keyboardShift, { toValue: 0, duration: 150, useNativeDriver: true }).start();
      NavigationBar.setVisibilityAsync('visible').catch(() => {});
    };

    const showSub = Keyboard.addListener(showEventName, onShow);
    const hideSub = Keyboard.addListener(hideEventName, onHide);
    return () => {
      showSub.remove();
      hideSub.remove();
    };
  ...
  useEffect(() => {
    ...
    const loop = Animated.loop(
      Animated.sequence([
        Animated.timing(micPulse, { toValue: 1.15, duration: 550, useNativeDriver: true }),
  ...
  return (
    <Modal visible={modalVisible} animationType="none" transparent onRequestClose={requestClose}>
      <Animated.View style={[cs.backdrop, { opacity: backdropOpacity }]}>
        ...
        {/* panelWrap تابت الحجم والموضع (padding ثابتة من الأربع
            جهات) — البطاقة (panel) جواه هي اللي بتاخد حجمها الفعلي
            (PANEL_HEIGHT × width محدود بـ PANEL_MAX_WIDTH). pointerEvents
            box-none عشان لمسة برّه البطاقة (بس جوه مساحة الـ padding)
            تفضل تقفل الشات زي المتوقع بدل ما panelWrap ياخدها لنفسه. */}
        <View style={cs.panelWrap} pointerEvents="box-none">
          {/* رجّعنا رفع البطاقة كلها بـ translateY (بدل تحريك height)
              عشان ده الأسلوب المُثبت اللي بيمنع باگ اختفاء/تلاشي
              الرسايل وقت الكتابة. */}
          <Animated.View
            style={[
              cs.panel,
          ...
          >
            {/* طبقة الزجاج الضبابي: بتبيّن عناصر الشاشة اللي وراء الشات
                بضبابية بدل خلفية صلبة — ده اللي بيدّي إحساس "الكارت
                الزجاجي الشفاف" المطلوب. */}
            <View style={[StyleSheet.absoluteFillObject, cs.panelGlassTint]} />

            <View style={cs.panelContent}>
            {/* ===== Header ===== */}
            ...
            <View style={cs.inputRow}>
              ...
              <TextInput
                ...
                placeholder={isRecording ? 'بنسمعك...' : 'اكتب رسالتك...'}
...
const cs = StyleSheet.create({
  // الخلفية خلف البانل: برغندي شفاف خفيف (مش أسود تقيل) عشان الشريط
  // الفاضي فوق البانل (بارتفاع الهيدر) يبان فيه هيدر المتجر معتّم
  // خفيف بدل أسود تقيل — "quiet luxury" مش "modal يقفل المتجر تمامًا".
  backdrop: { flex: 1, backgroundColor: 'rgba(107,30,46,0.22)' },
  // panelWrap مساحته padding ثابتة من الأربع جهات (زي left/right/top/
  // bottom الثابتين في نسخة الويب) — هو مجرد "إطار تمركيز" للبطاقة،
  // مش هو نفسه البانل. البطاقة (panel) جواه بتاخد حجمها الفعلي الثابت.
  panelWrap: {
    flex: 1,
    alignItems: 'center',
    paddingTop: PANEL_TOP_OFFSET,
    paddingBottom: PANEL_BOTTOM_OFFSET,
    paddingHorizontal: PANEL_SIDE_MARGIN,
  },
  // البطاقة بقت عائمة بحجم ثابت (width كامل العرض المتاح بحد أقصى
  // PANEL_MAX_WIDTH، وheight = PANEL_HEIGHT الثابت المحسوب فوق) —
  // زوايا مدوّرة بالكامل من الأربع جهات (PANEL_BORDER_RADIUS) بدل
  // الزوايا العلوية المربّعة القديمة بتاعة وضع الـ docking.
  // البطاقة بقت زجاجية شبه شفافة (مش خلفية صلبة) — الـ backgroundColor
  // اتشال خالص من هنا، وBlurView + panelGlassTint (جوه الـ JSX) هم
  // اللي بيدّوا لون/ضبابية الخلفية دلوقتي، عشان عناصر الشاشة اللي وراء
  // الشات تبان من خلاله بضبابية زي الزجاج المطلوب.
  panel: {
    width: '100%',
    ...
    overflow: 'hidden',
    // خلفية صلبة احتياطية (fallback) تظهر فورًا تحت البلور مباشرة —
    // لو الـ BlurView فشل يشتغل على أي جهاز (زي ما حصل على أندرويد)،
    // الكارت يفضل بطاقة واضحة بلون ثابت بدل ما يبقى شفاف بالكامل
    // ويوري صفحة المتجر اللي وراه (وده اللي كان بيخفي كتابة الرسايل).
  ...
  // تينت دافئ خفيف جدًا فوق البلور (بدل خلفية عاجية صلبة) — بيحافظ على
  // هوية ألوان ATEEM لكن يسيب الضبابية والشفافية بايناه من وراه.
  panelGlassTint: { backgroundColor: 'rgba(245,239,230,0.28)' },
  // الحاوية الحقيقية للمحتوى (هيدر + رسايل + إدخال) فوق طبقتي البلور
  // والتينت، بارتفاع كامل البطاقة.
  panelContent: { flex: 1 },

  // الهيدر بقى أقل ارتفاعًا وبخلفية عنابية شبه شفافة (بدل صلبة) عشان
  // يبقى متناسق مع باقي الكارت الزجاجي، مع سطوع كافي إن النص الأبيض
  // يفضل واضح فوقه.
  header: {
    // بقى صلب تمامًا (مش شفاف) بناءً على الطلب — الشفافية/الضبابية
    // اتقصرت على وسط الشات (منطقة الرسايل) بس، مش الهيدر.
    ...
    borderBottomColor: 'rgba(201,169,97,0.55)',
  },
  headerContent: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  // الأفاتار والحلقة الذهبية حواليه اتصغّروا شوية (38 بدل 46) زي
  // المطلوب — لسه واضحين لكن مايشغلوش مساحة كبيرة من الهيدر الأقصر.
  logoImgWrap: {
  ...
  logoImg: { width: 38, height: 38, borderRadius: 19 },
  title: { fontFamily: THEME.fontHeading, color: COLORS.white, fontSize: 15, letterSpacing: 0.3, textAlign: 'right' },
  statusDot: {
    position: 'absolute', bottom: 0, right: 0,
    width: 9, height: 9, borderRadius: 4.5,
    backgroundColor: '#4CAF50',
    borderWidth: 1.5, borderColor: COLORS.burgundy,
  },
  // زرار الإغلاق اتصغّر شوية (36 بدل 44) بما يتناسب مع الهيدر الأقصر.
  closeBtn: {
  ...
  // خلفية جسم الرسايل شفافة تمامًا دلوقتي — البلور جاي من البطاقة نفسها
  // (panel)، مش من هنا، عشان الضبابية تكون موحّدة على الكارت كله.
  ...
  // صف رسالة البوت — بلا أفاتار جنب كل رسالة (شيلناه بناءً على الطلب)،
  // بس فقاعة الرد. maxWidth 80% زي المواصفات السابقة.
  botRow: { maxWidth: '80%', alignSelf: 'flex-start' },
  botCol: { flex: 1 },
  botBubble: {
  ...
  // أزرار الإجراءات السريعة: تلاتة بس، في صف عرضي واحد (مش شبكة)،
  // وكل كبسولة بتاخد نفس العرض بالظبط (flex:1) — زي المطلوب بالظبط.
  optionsWrap: { flexDirection: 'row-reverse', flexWrap: 'nowrap', gap: 8, marginTop: 10 },
  optionPill: {
    flex: 1,
    alignItems: 'center', justifyContent: 'center',
  ...
  // adjustsFontSizeToFit (مضبوطة في الـ JSX) بتصغّر الخط تلقائيًا لو
  // النص الأطول ("العروض الحالية") مش هيتسع في سطر واحد بالعرض المتاح.
  optionPillText: { fontFamily: THEME.fontBodySemiBold, fontSize: 12.5, color: COLORS.burgundy, textAlign: 'center' },

  // حاوية الإدخال بقت أقل ارتفاعًا (paddingVertical أقل) عشان يتساوى
  // ارتفاعها تقريبًا مع ارتفاع الهيدر الجديد الأقصر، وخلفية شبه شفافة
  // (بدل صلبة) عشان تفضل متسقة مع خلفية البطاقة الزجاجية.
  inputRow: {
    flexDirection: 'row', alignItems: 'center', gap: 8,
    paddingHorizontal: 12, paddingVertical: 7,
    // بقت صلبة تمامًا (مش شفافة) بناءً على الطلب — نفس منطق الهيدر.
    backgroundColor: THEME.card,
  ...
  // from the chat background")، وارتفاعه اتقلل شوية (paddingVertical
  // 8 بدل 10) بما يتناسب مع الحاوية الأقصر.
  input: {
    flex: 1, backgroundColor: '#FFFDF9', borderWidth: 1, borderColor: 'rgba(201,169,97,0.35)', borderRadius: 20,
    paddingHorizontal: 16, paddingVertical: 8, fontFamily: THEME.fontBody, fontSize: 14, color: THEME.textPrimary,
  },
  // زر موحّد (مايكروفون ↔ إرسال) اتصغّر شوية (36 بدل 40) بما يتناسب
  // مع حاوية الإدخال الأقصر — حد ذهبي رفيع في وضعه الافتراضي (مايك)
  // بدل الأيقونة العارية، وبيتحول لدائرة عنابية معبأة وقت التسجيل
  // الفعلي أو وجود نص جاهز للإرسال.
  actionBtn: {
    width: 36, height: 36, borderRadius: 18,
    alignItems: 'center', justifyContent: 'center',
