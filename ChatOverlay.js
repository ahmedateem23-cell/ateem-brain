...
export default function ChatOverlay({ visible, onClose, products }) {
  ...
  // (رجّعنا الأسلوب المُثبت ده تحديدًا لأن تحريك height مباشرة سبب
  // باگ اختفاء/"تلاشي" الرسايل وقت الإرسال والاستقبال — الكود الأصلي
  // كان فعلاً محوّل من نفس الأسلوب ده قبل كده للسبب ده بالظبط). حجم
  // البطاقة (PANEL_HEIGHT) ثابت دايمًا. لما الكيبورد يظهر، بنحسب بس
  // قد ايه هو مغطّي من المساحة السفلية المحجوزة أصلاً
  ...
  useEffect(() => {
    ...
    const onHide = () => {
      Animated.timing(keyboardShift, { toValue: 0, duration: 180, useNativeDriver: true }).start();
    };
        ...
        <View style={cs.panelWrap} pointerEvents="box-none">
          ...
          <Animated.View
            style={[
              ...
              {
                transform: [
                  { translateY: Animated.subtract(panelTranslateY, keyboardShift) },
                  { scale: panelScale },
                ],
              },
            ]}
...
const cs = StyleSheet.create({
  ...
  // يفضل واضح فوقه.
  header: {
    backgroundColor: 'rgba(107,30,46,0.62)',
    minHeight: 56,
  ...
  inputRow: {
    ...
    backgroundColor: 'rgba(255,253,249,0.35)',
