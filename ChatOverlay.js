...
// الشات دلوقتي "مرسو" (docked) تحت هيدر المتجر الرئيسي مباشرة، مش
// بطاقة عائمة بارتفاع ثابت زي قبل. PANEL_TOP_OFFSET = ارتفاع status
// bar + ارتفاع navbar المتجر في App.js (paddingVertical:10*2 + محتوى
// العلامة التجارية ≈ 64px) — نفس الرقمين دول لازم يتطابقوا مع تصميم
// الهيدر الرئيسي عشان الشات يبدأ بالظبط من تحته من غير فجوة أو تراكب.
const STATUS_BAR_HEIGHT = Platform.OS === 'android' ? (RNStatusBar.currentHeight || 24) : 44;
const STORE_HEADER_HEIGHT = 64;
const PANEL_TOP_OFFSET = STATUS_BAR_HEIGHT + STORE_HEADER_HEIGHT;
const BOTTOM_SAFE_MARGIN = 16;
// أقل ارتفاع مسموح للبانل يوصله لما يطوي من تحت وقت ظهور الكيبورد —
// حماية من إن لوحة مفاتيح طويلة جدًا تقفل البانل خالص.
...
export default function ChatOverlay({ visible, onClose, products }) {
  ...
  // ===== طيّ البانل من تحت وقت ظهور الكيبورد، مع تثبيت الأعلى =====
  // الأعلى ثابت دايمًا بفضل marginTop: PANEL_TOP_OFFSET (رقم صريح مش
  // متحرك أبدًا). لما الكيبورد يظهر، بنضيف paddingBottom متحرك بارتفاع
  // الكيبورد نفسه على panelWrap (flex:1) — ده بيقلل المساحة المتاحة
  // للبانل من تحت بس، فيبان "بيطوي" من تحت، من غير ما يتحرك من فوق
  // خالص. بحد أقصى (clamp) بيمنع الطيّ إنه يوصل لأقل من ارتفاع مفيد.
  const keyboardPad = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    const availableHeight = SCREEN_HEIGHT - PANEL_TOP_OFFSET - BOTTOM_SAFE_MARGIN;
    const maxPad = Math.max(0, availableHeight - MIN_PANEL_HEIGHT);
    ...
    const onShow = (e) => {
      ...
      const clamped = Math.min(keyboardHeight, maxPad);
    ...
    const onHide = () => {
      Animated.timing(keyboardPad, { toValue: 0, duration: 180, useNativeDriver: false }).start();
    };
  ...
  return (
    <Modal visible={modalVisible} animationType="none" transparent onRequestClose={requestClose}>
      <Animated.View style={[cs.backdrop, { opacity: backdropOpacity }]}>
        ...
        {/* panelWrap ثابت من فوق دايمًا (marginTop: PANEL_TOP_OFFSET) —
            paddingBottom بس هو المتحرك (keyboardPad)، فالطيّ كله بيحصل
            من تحت والأعلى مايتزحزحش خالص. */}
        <Animated.View style={[cs.panelWrap, { paddingBottom: keyboardPad }]}>
          <Animated.View
            style={[
              cs.panel,
...
const cs = StyleSheet.create({
  ...
  // البانل بقى "مرسو" (docked) تحت هيدر المتجر مباشرة، مش بطاقة عائمة:
  // marginTop ثابت بالظبط = ارتفاع الهيدر الرئيسي (PANEL_TOP_OFFSET)،
  // وflex:1 عشان يمتد لآخر الشاشة. paddingBottom بتاع الكيبورد بيتحط
  // هنا (شوف الـ JSX) فيقلل المساحة من تحت بس، والأعلى ثابت مايتزحزحش.
  panelWrap: { flex: 1, marginTop: PANEL_TOP_OFFSET },
  panel: {
    flex: 1,
    backgroundColor: THEME.background,
    // حواف علوية مربّعة (البانل ملزّق في الهيدر مباشرة من غير فجوة)،
    // وحواف سفلية مدوّرة لمسة نهائية أنيقة.
