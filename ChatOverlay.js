  ...
  return (
    <Modal visible={modalVisible} animationType="none" transparent onRequestClose={requestClose}>
      <Animated.View style={[cs.backdrop, { opacity: backdropOpacity }]}>
        ...
        {/* behavior="position" على المنصتين بيرفع الـ View بالكامل
            بالـ translate (مش بيغيّر ارتفاعه)، فالبانل بيحافظ على
            حجمه بالظبط ويرتفع بس — بدل "padding"/"resize" اللي كانت
            بتخلي نسبة الـ 70% تتحسب من مساحة أصغر بعد فتح الكيبورد
            فيبان البانل "بينكمش". لازم كمان تتأكد إن android في
            app.json مظبوط على "pan" مش "resize" (شوف الملاحظة تحت). */}
        <KeyboardAvoidingView
          behavior="position"
          style={cs.panelWrap}
        >
