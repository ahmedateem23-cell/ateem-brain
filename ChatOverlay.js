...
// مسافة ثابتة من أعلى الشاشة (status bar + هامش بسيط) قبل بداية
// البطاقة — زي top:64px في نسخة الويب.
const PANEL_TOP_OFFSET = STATUS_BAR_HEIGHT + 20;
...
// أقصى مسافة مسموح نرفع بيها البطاقة فوق وقت ظهور الكيبورد، عشان
// حافتها العلوية ما تقربش أوي من status bar.
...
// أربعة أزرار سريعة بس (مطابقة للمواصفات بالظبط)، بدل التلاتة
// القدام — "لا تجعل الأزرار كثيرة" في البريف بيقصد بيها عدم الإضافة
// عليهم، مش تقليل العدد المطلوب صراحةً.
const UTILITY_ACTIONS = [
  { label: 'اكتشف المنتجات', text: 'أرني أبرز المنتجات المتوفرة عندكم' },
  { label: 'مساعدة في اختيار المقاس', text: 'محتاج مساعدة في اختيار المقاس المناسب' },
...
export default function ChatOverlay({ visible, onClose, products }) {
  ...
  // حجم البطاقة (PANEL_HEIGHT) ثابت دايمًا. لما الكيبورد يظهر، بنحسب
  // بس قد ايه هو مغطّي من المسافة السفلية المحجوزة أصلاً (PANEL_BOTTOM_OFFSET)
  // ولو غطّاها بنرفع البطاقة كلها لفوق بنفس الفرق (translateY)، تمامًا
  // زي ما بيحصل تلقائيًا في نسخة الويب لما الـ viewport يتقلّص. ده
  // بيمنع مشكلة "تمدد"/تقلّص البانل اللي كانت موجودة قبل كده.
...
const cs = StyleSheet.create({
  ...
  // الهيدر بقى بخلفية عنابية (Burgundy header فوق Ivory body) بالظبط
  // زي المواصفات — بدل الهيدر العاجي القديم. باقي التفاصيل (الحد
  // الذهبي الرفيع تحت، ونقطة "متصل الآن") اتعدّلت عشان تفضل واضحة
  // فوق الخلفية العنابية الجديدة.
  header: {
  ...
  // اليمنى من الأفاتار بدل ما تفضل سطر نص منفصل. حلقة ذهبية رفيعة
  // جدًا حوالين الأفاتار — لمسة Champagne Gold صغيرة زي المطلوب.
  logoImgWrap: {
  ...
  title: { fontFamily: THEME.fontHeading, color: COLORS.white, fontSize: 17, letterSpacing: 0.3, textAlign: 'right' },
  statusDot: {
  ...
  // زرار الإغلاق: دايرة زجاجية شفافة خفيفة بحد ذهبي رفيع، عشان تبان
  // بوضوح فوق الهيدر العنابي الجديد (بدل الخلفية العنابية القديمة اللي
  // كانت هتختفي جوه هيدر عنابي).
  closeBtn: {
  ...
  // أزرار الإجراءات السريعة: شبكة 2×2 هادئة (بدل صف واحد مزدحم بأربع
  // عناصر) — flexBasis:'48%' بيخلي كل زرار ياخد تقريبًا نص العرض
  // فيتكوّن صفين متوازيين، وده أهدأ وأرقى من ضغط 4 pills في سطر واحد.
  ...
  // النص الأطول ("سياسة التوصيل") مش هيتسع في سطر واحد بالعرض المتاح.
