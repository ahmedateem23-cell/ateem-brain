...
// ارتفاع الشاشة بيتقرا مرة واحدة بس هنا وقت تحميل الملف — قبل أي فتح
// للكيبورد. لو استخدمنا Dimensions.get() جوه StyleSheet مباشرة أو
// height:'70%' (نسبة)، لما الكيبورد يفتح وأندرويد يصغّر نافذة
// التطبيق (وضع resize الافتراضي)، النسبة كانت بتتحسب من الارتفاع
// الجديد الأصغر فيبان البانل بيـ"ينكمش". برقم ثابت بالبكسل زي ده،
// البانل بيحافظ على حجمه الفعلي دايمًا بغض النظر عن الكيبورد.
...
export default function ChatOverlay({ visible, onClose, products }) {
  ...
  // ===== رفع البانل فوق الكيبورد يدويًا (بدل الاعتماد على تصغير
  // أندرويد التلقائي للنافذة أو KeyboardAvoidingView) =====
  // بنستمع لارتفاع الكيبورد الحقيقي ونرفع البانل بنفس المقدار، لكن
  // بحد أقصى (clamp) يضمن إن البانل ميوصلش أبداً فوق الهيدر حتى لو
  // الكيبورد كان طويل جداً (لوحات المفاتيح العربية بعض الأحيان بتاخد
  // 35-40% من الشاشة). المسافة الأصلية فوق البانل قبل أي رفع تقريبًا
  // (SCREEN_HEIGHT - PANEL_HEIGHT)، فبنسيب هامش أمان بسيط منها.
  const keyboardOffset = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    const SAFE_TOP_MARGIN = 24;
    const originalTopGap = SCREEN_HEIGHT - PANEL_HEIGHT;
    const maxUpwardOffset = Math.max(0, originalTopGap - SAFE_TOP_MARGIN);
    ...
    const onShow = (e) => {
      ...
      const clamped = Math.min(keyboardHeight, maxUpwardOffset);
  ...
  return (
    <Modal visible={modalVisible} animationType="none" transparent onRequestClose={requestClose}>
      <Animated.View style={[cs.backdrop, { opacity: backdropOpacity }]}>
        ...
        {/* View عادية بدل KeyboardAvoidingView — الرفع فوق الكيبورد
            بقى بالكامل يدوي عبر keyboardOffset (شوف useEffect فوق)،
            بحد أقصى مضمون بيمنع البانل يوصل فوق الهيدر مهما كان طول
            الكيبورد، وده بيشتغل بشكل موحّد وثابت على المنصتين من غير
            ما يعتمد على سلوك تصغير نافذة أندرويد التلقائي (اللي كان
            سبب كل المشاكل السابقة). */}
        <View style={cs.panelWrap}>
          <Animated.View
            style={[
              cs.panel,
              { transform: [{ translateY: panelTranslateY }, { scale: panelScale }, { translateY: keyboardOffset }] },
            ]}
          >
            {/* ===== Header ===== */}
